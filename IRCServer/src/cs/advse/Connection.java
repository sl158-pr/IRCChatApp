...
/*Internet Chat Room Project submitted for CSE6324
 * Student ID- 1000903791,100915823, 1000830230
 * Name-Sumanth L
 * 
 * This is the server side code for the mIRC Client app, which parses the incoming Command requests and sends Numerical replies 
 * It creates a Connection thread, Channel class, enum of Command
 */
...
public class Connection implements Runnable
{
	...
	private static String globalServerName="localhost";
	...
	public enum Command
	{
		NICK(1, 1)		//Command- /NICK <nickname>
		{
			@Override
		...
		USER(1, 4)		//this command called implicitly
		{
			@Override
			public void run(Connection con, String prefix, String[] arguments)
					throws Exception
		...
		JOIN(1, 2)		//Command- /JOIN #Channelname
		{
		...
		WHO(0, 2)	/* Command is- /WHO #Channelname*/
		{
			@Override
			public void run(Connection con, String prefix, String[] arguments)
					throws Exception
		...
		USERHOST(1, 5)		//this comand is called implicitly
		{
			@Override
			public void run(Connection con, String prefix, String[] arguments)
					throws Exception
		...
		PART(1, 2)		//Command- /PART #channelname
		{
			@Override
			public void run(Connection con, String prefix, String[] arguments)
					throws Exception
		...
		PRIVMSG(2, 2)		//this command is called when we double click user in the nickname list 
		{
			@Override
			public void run(Connection con, String prefix, String[] arguments)
					throws Exception
		...
		KICK(1,2)		//Command- /KICK <#Channelname> <username>
		{
			...
			public void run(Connection con, String prefix, String[] arguments)
					...
					synchronized (mutex)
					{
						Connection kickedCon=connectionMap.get(arguments[1]);
						...
						{
//							con.sendKick(arguments[1], kickedCon);
							for (String channelName : new ArrayList<String>(channelMap.keySet()))
							{
								Channel channels = channelMap.get(channelName);
								channels.send(":" + " KICK "	+ arguments[1]);
								channels.channelMembers.remove(kickedCon);								
								if (channels.channelMembers.size() == 0)
									channelMap.remove(channel.name);
								for (Connection channelMember : channels.channelMembers)
								{// 353,366
									channels.channelMembers.remove(kickedCon);	
									con.sendGlobal("353 " + con.nick + " = " + arguments[0]
											+ " :" + channelMember.nick);
								}
								con.sendGlobal("366 " + con.nick + " " + channelName
										+ " :End of /NAMES list");
...
//							channel.send(": KICK " + arguments[1]);
//							channel.channelMembers.remove(kickedCon);
//							if (channel.channelMembers.size() == 0)
...
//				}
		...
		TOPIC(1, 2)		//Command- /TOPIC <#channelname> <newtopic>
		{
			@Override
			public void run(Connection con, String prefix, String[] arguments)
					throws Exception
...
//	protected void sendKick(String quitMessage,Connection kicked)
//	{
//		synchronized (mutex)
//		{
//			for (String channelName : new ArrayList<String>(channelMap.keySet()))
//			{
//				Channel channel = channelMap.get(channelName);
//				channel.send(":" + " KICK "	+ quitMessage);
//				channel.channelMembers.remove(kicked);
//				
//				if (channel.channelMembers.size() == 0)
//					channelMap.remove(channel.name);
//				
